...
- (void)processTouchAtPoint:(CGPoint)touchPoint {
    ...
    NSInteger markerId = [self.graphicsCore markerIdWithTouchPoint:touchPoint];
    
    [self.graphicsCore selectPlotAtMarkerId:markerId];
    [self.delegate didSelectPlot:[self plotForMarkerId:markerId]];
